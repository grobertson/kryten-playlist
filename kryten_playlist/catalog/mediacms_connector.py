...
class MediaCMSConnector:
    ...
    token: str | None = None
    """API Token for authentication (X-USER-TOKEN header)."""

    def get_auth_headers(self) -> dict[str, str]:
        """Get authentication headers."""
        headers = {}
        if self.token:
            # MediaCMS/DRF usually expects "Authorization: Token <token>"
            # We also keep X-USER-TOKEN as it was used previously, but Authorization is standard.
            # Check if token already has a prefix
            if " " in self.token:
    ...
    async def iter_items(self) -> AsyncIterator[CatalogItem]:
        ...
        async with httpx.AsyncClient(timeout=self.timeout, auth=auth, headers=headers) as client:

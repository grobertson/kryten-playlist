# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.3.2] - 2025-12-26

### Removed
- Removed `backfill` command and related functionality due to MediaCMS API limitations.

### Fixed
- Fixed critical test failures in `test_enrich.py` and `test_queue_apply.py`.
- Resolved connection errors in test suite.
- Fixed `MockKrytenClient` missing `send_command` method.

## [0.3.1] - Unreleased

### Added
- Initial skeleton implementation
- Basic service structure with KrytenClient integration
- Event handlers for `chatMsg` and `addUser` events
- Configuration management system
- CI workflow with Python 3.10, 3.11, and 3.12 support
- PyPI publishing workflow with trusted publishing
- Startup scripts for PowerShell and Bash
- Systemd service manifest
- Documentation structure

### Changed

### Deprecated

### Removed

### Fixed

### Security

## [0.1.0] - Unreleased

Initial development release.
